...
out_path = str(inp.with_name(f"{inp.stem}_result.jpg"))
image = Image.open(image_path)

text_prompts = ["ports", "port"]
print("prompts:", text_prompts)
...
def save_overlay(image, masks, boxes, scores, path, alpha=0.5, labels=None):
...
for r in all_results:
    k = nobj(r["masks"])
    labels.extend([r["prompt"]] * k)

t2 = time.perf_counter()
save_overlay(image, masks, boxes, scores, out_path, labels=labels)
